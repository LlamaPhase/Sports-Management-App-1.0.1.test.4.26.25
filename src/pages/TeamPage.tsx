...
import { Plus, Shield, ChevronDown, Loader2 } from 'lucide-react'; // Added Loader2
import { TeamContext, Player, Game, PlayerLineupState, GameEvent, PlayerData } from '../context/TeamContext'; // Import PlayerData
...
const TeamPage: React.FC = () => {
  // Consume players, playersLoading, and CRUD functions from context
  const { players, playersLoading, addPlayer, updatePlayer, deletePlayer, games, teamData, selectGame, gameHistory } = useContext(TeamContext);
  const [isAddModalOpen, setIsAddModalOpen] = useState(false);
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);
  const [playerToEdit, setPlayerToEdit] = useState<Player | null>(null); // Still uses local Player type for modal
  ...
  // --- Memoized Data (Unchanged logic, uses context `players`) ---
  const sortedPlayers = useMemo(() =>
    [...players].sort((a, b) => a.first_name.localeCompare(b.first_name)), // Use first_name from Player type
    [players]
  );
  ...
  // NEW: Wrapper for addPlayer to handle async nature if needed (though modal closes itself)
  const handleAddPlayerSubmit = async (firstName: string, lastName: string, number: string) => {
      ...
      // Modal closes itself, no need to setIsAddModalOpen(false) here unless addPlayer fails
  };
  // NEW: Wrapper for updatePlayer
  const handleUpdatePlayerSubmit = async (id: string, updates: Partial<Pick<PlayerData, 'first_name' | 'last_name' | 'number'>>) => {
      ...
      handleCloseEditModal(); // Close modal after update attempt
  };
   // NEW: Wrapper for deletePlayer
   const handleDeletePlayerSubmit = async (id: string) => {
       await deletePlayer(id);
       handleCloseEditModal(); // Close modal after delete attempt
   };
                  ...
                  return (
                    <button key={player.id} onClick={() => handleEditPlayerClick(player)} className="flex items-center p-2 border-b last:border-b-0 w-full text-left hover:bg-gray-50 transition rounded text-sm">
                      <div className="w-1/4 flex items-center space-x-2 pr-2">
                        {/* PlayerIcon now receives the Player type directly */}
