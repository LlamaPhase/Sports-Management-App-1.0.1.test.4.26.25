...
import { TeamContext, Player, Game, PlayerLineupState, GameEvent, PlayerData } from '../context/TeamContext';
...
const calculatePlayerStats = (playerId: string, filteredGames: Game[]): PlayerStats => {
  let pt = 0;
  let ga = 0;
  let gp = 0;
  let s = 0;
  ...
  filteredGames.forEach(game => {
    if (game.is_explicitly_finished) {
      const playerState = game.lineup?.find(p => p.id === playerId);
      if (playerState) {
        if (playerState.location === 'field' || playerState.location === 'bench') ga++;
  ...
  return { pt, ga, gp, s, g, a, yc: 0, rc: 0 }; // YC/RC placeholders
};

const TeamPage: React.FC = () => {
  const {
    players, playersLoading, addPlayer, updatePlayer, deletePlayer,
    games, gamesLoading, teamData, selectGame, gameHistory
  ...
  const upcomingGames = useMemo(() =>
    validGames.filter(game => {
      ...
      return game.timer_status !== 'running' && !isFinished && gameDateTime >= now;
    }).sort((a, b) => new Date(`${a.game_date}T${a.game_time || '00:00:00'}`).getTime() - new Date(`${b.game_date}T${b.game_time || '00:00:00'}`).getTime()),
    [validGames, now]
  );

  const previousGames = useMemo(() =>
    validGames.filter(game => game.is_explicitly_finished === true)
      .sort((a, b) => new Date(`${b.game_date}T${b.game_time || '00:00:00'}`).getTime() - new Date(`${a.game_date}T${a.game_time || '00:00:00'}`).getTime()),
    [validGames]
  );
  ...
  const filteredGamesForStats = useMemo(() => {
    return validGames.filter(game => {
      ...
      return game.is_explicitly_finished === true && seasonMatch && competitionMatch;
    });
  }, [validGames, selectedSeason, selectedCompetition]);

  const availableCompetitions = useMemo(() => {
    if (!selectedSeason) return gameHistory.competitions;
  ...
  }, [selectedSeason, validGames, gameHistory.competitions]);

  useEffect(() => {
    if (selectedCompetition && !availableCompetitions.includes(selectedCompetition)) {
      setSelectedCompetition('');
    }
  }, [selectedSeason, selectedCompetition, availableCompetitions]);
  ...
  const handleAddPlayerSubmit = async (firstName: string, lastName: string, number: string) => { await addPlayer(firstName, lastName, number); };
  const handleUpdatePlayerSubmit = async (id: string, updates: Partial<Pick<PlayerData, 'first_name' | 'last_name' | 'number'>>) => { await updatePlayer(id, updates); handleCloseEditModal(); };
  const handleDeletePlayerSubmit = async (id: string) => { await deletePlayer(id); handleCloseEditModal(); };

  return (
    <div className="space-y-6">
      {/* Next Game Section */}
      {nextGame && (
        <button onClick={() => selectGame(nextGame.id)} className="w-full text-left bg-white p-4 rounded-lg shadow hover:bg-gray-50 transition">
          <h2 className="text-lg font-semibold mb-3 text-gray-700">Next Game</h2>
          <p className="text-sm text-gray-500 mb-3">
            {formatDate(nextGame.game_date, { weekday: 'short', month: 'short', day: 'numeric' })}
          </p>
          <div className="flex justify-center items-center space-x-2">
            <TeamDisplay name={nextGame.location === 'home' ? currentTeamName : nextGame.opponent} logo={nextGame.location === 'home' ? currentTeamLogo : null} isOpponentTeam={nextGame.location === 'away'} className="flex-1 justify-end text-right" />
            <span className="text-sm font-semibold text-gray-800 px-1 flex-shrink-0 w-16 text-center">
              {formatTime(nextGame.game_time)}
            </span>
            <TeamDisplay name={nextGame.location === 'away' ? currentTeamName : nextGame.opponent} logo={nextGame.location === 'away' ? currentTeamLogo : null} isOpponentTeam={nextGame.location === 'home'} className="flex-1 justify-start text-left" />
          </div>
        </button>
      )}
        ...
        <div className="bg-white p-4 rounded-lg shadow">
          ...
          <div className="flex justify-around items-start space-x-2 text-center">
            {lastFiveGamesReversed.map((game) => {
              ...
              return (
                <button key={game.id} onClick={() => selectGame(game.id)} className="flex flex-col items-center space-y-1 flex-1 min-w-0 hover:opacity-75 transition">
      ...
      <div className="bg-white p-4 rounded-lg shadow">
        ...
        {!playersLoading && !gamesLoading && sortedPlayers.length > 0 && (
          <p className="text-xs text-gray-500 mt-3 text-center">
            Note: Yellow Cards (YC) and Red Cards (RC) are currently placeholders.
          </p>
        )}
      </div>
